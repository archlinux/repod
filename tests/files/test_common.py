...
async def test_open_package_file(
    ...
    match file_type:
        ...
        case ".foo":
            path = Path("foo.foo")
    with expectation:
        assert await package.open_package_file(path=path)


async def test_extract_from_package_file(zst_file: Path) -> None:
    with package.ZstdTarFile(zst_file) as tarfile:
        for member in tarfile.getmembers():
            if member.name.endswith(".txt"):
                assert await package.extract_from_package_file(tarfile, member.name)
            else:
                with raises(RepoManagementFileNotFoundError):
                    assert await package.extract_from_package_file(tarfile, member.name)

                with raises(RepoManagementFileNotFoundError):
                    assert await package.extract_from_package_file(tarfile, f"{member.name}foobar")


def test_zstdtarfile_raises(zst_file: Path) -> None:
    with patch.object(package.TarFile, "__init__") as tarfile_mock:
        tarfile_mock.side_effect = Exception("FAIL")
        with raises(RepoManagementFileError):
            package.ZstdTarFile(name=zst_file, mode="r")
